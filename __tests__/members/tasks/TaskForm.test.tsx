...
describe("Test task form component", () => {
  ...
  it("does not render the form when showingModal is false", () => {
  ...
  it("should populate fields with task data", () => {
    renderedComponent(true, mockUserTasks[0]);
    expect(
      screen.getByDisplayValue(mockUserTasks[0].title),
    ).toBeInTheDocument();
    if (mockUserTasks[0].description) {
      expect(
        screen.getByDisplayValue(mockUserTasks[0].description),
      ).toBeInTheDocument();
    }
  });

  it("calls onVisibilityChange when the Close button is clicked", () => {
