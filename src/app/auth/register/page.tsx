  ...
  const [error, submitLogin, isPending] = useActionState(
    async (_previousState: ZodIssue[] | null, formData: FormData) => {
      ...
      if (validatedData.success) {
        ...
        if (response.status === "success") redirect("/auth/login");
        if (Array.isArray(response.error)) return response.error;
        return [
          {
      ...
      } else return validatedData.error.issues;
    },
    null,
  );
      ...
      <form action={submitLogin}>
        ...
        <div className="pt-5 w-full">
          ...
          >
            {isPending && (
              <span
                className={clsx("loading loading-spinner", {
                  invisible: !isPending,
                })}
              >
                {" "}
        ...
        <div className="text-sm md:text-base  flex flex-col pt-5">
          ...
          {error &&
            error.map((issue) => (
              <span key={issue.code} className="text-orange-300 text-left">
                - {issue.message}
              </span>
            ))}
        </div>
