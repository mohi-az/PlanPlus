...
export default function CategoriesList() {
  ...
  const { categories, addCategory, isPending, updateCategory, deleteCategory } =
  ...
  const handleSubmit = async (formData: FormData) => {
    const categoryData = {
      ...
      icon: selectedIcon,
      showInMenu: formData.get("showInMenu") === "on" ? true : false,
    };
  ...
  const deleteHandler = async (cat: TaskCategory) => {
  ...
  return (
    <div className="overflow-x-auto  flex flex-col  lg:flex-row gap-6 h-full">
      <form
        action={handleSubmit}
        className="flex flex-col  form-control gap-2 px-5  w-full lg:w-1/3 h-2/5 lg:h-full  rounded-md bg-base-200"
      >
        <div className="label">
          ...
          <div className=" flex flex-row text-base md:text-2xl lg:text-4xl gap-1 md:gap-2 flex-wrap justify-center">
            {iconNames.map((i) => (
              <div
                key={i}
                className={clsx(
                  "border-1 cursor-pointer border-slate-500 hover:border-primary",
                  (selectedIcon === i || i === selectedCategory?.icon) &&
        ...
        <div className="flex lg:flex-col flex-row  gap-4 justify-end  pb-10">
          <button className="btn btn-accent btn-outline w-1/3 lg:w-full btn-sm md:btn-md">
            {selectedCategory ? "Update" : "Add Category"}
          </button>
          {selectedCategory && (
            <button
              type="button"
              className="btn btn-ghost btn-outline w-1/3 lg:w-full btn-sm md:btn-md "
        ...
        {validationErrors.map((error) => (
          <span key={error} className="text-error text-sm">
            {error}
          </span>
        ))}
      </form>
