...
export default function TaskForm({
  ...
  const [status, addNewTask, isPending] = useActionState(
    async (
      __preState: string | ZodIssue[] | null,
      formData: FormData,
    ): Promise<string | ZodIssue[] | null> => {
      ...
      if (reminderDate && reminderTime)
        reminder = new Date(
          reminderDate?.toString() + "T" + reminderTime?.toString(),
        );
  ...
  useEffect(() => {
    setCategory(task?.category?.id ? task?.category?.id : "");
  }, [task]);
  return (
    <div>
      {showingModal && (
        <ModalForm title="Add New Task" isVisible={showingModal}>
          <form className="w-full " action={addNewTask} data-testid="taskForm">
            <div className="label">
            ...
            <input
              ...
              defaultValue={task?.description ? task.description : undefined}
            />
            ...
            <div className="collapse mt-3">
              ...
              <div className="collapse-content text-primary-content peer-checked:bg-primary peer-checked:text-secondary-content flex flex-row justify-items-end">
                <div className="w-2/3 flex flex-col md:flex-row gap-1 justify-center items-end">
                  <input
                    type="date"
                    name="reminderDate"
                    className="input text-white input-bordered w-full input-sm md:input-md"
                    aria-label="Task due date"
                    defaultValue={
                      task?.reminder
                        ? new Date(task.reminder?.remindAt)
                            .toISOString()
                            .split("T")[0]
                        : undefined
                    }
                  ...
                  <input
                    ...
                    defaultValue={
                      task?.reminder
                        ? new Date(task.reminder?.remindAt).toLocaleTimeString(
                            [],
                            {
                              hour: "2-digit",
                              minute: "2-digit",
            ...
            <div className="pt-5 w-full flex flex-row gap-4">
              <button
                ...
                className="btn btn-success btn-sm md:btn-md  w-1/2 flex flex-row"
                disabled={isPending}
              >
                {isPending && (
              ...
              <button
                type="reset"
                className=" btn btn-neutral btn-sm md:btn-md w-1/2"
