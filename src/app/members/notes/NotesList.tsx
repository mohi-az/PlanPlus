...
export default function NotesList() {
  ...
  const { notes, changeFavourite } = useContext(NotesContext);
  const [selectedNote, setSelectedNote] = useState("");
  const [favBTNActive, setFavBTNActive] = useState<string | null>(null);

  const changeFavouriteFlag = useCallback(
    (noteId: string) => {
      startTransition(async () => {
        await changeFavourite(noteId);
        setSelectedNote("");
          ...
          {notes &&
            notes.map((note, index) => (
              <tr key={note.id} className="w-full hover h-6 overflow-y-scroll">
                ...
                <td className=" hidden md:table-cell">
                  {" "}
                  {note.task.completeAt
                    ? new Date(note.task.completeAt).toDateString()
                    : ""}
                </td>
                <td>
                    ...
                    {isPending && selectedNote === note.id ? (
                      ...
                      <Image
                        ...
                        src={
                          note.isFavourite
                            ? "/images/favNote.png"
                            : favBTNActive === note.id
                              ? "/images/favNote.gif"
                              : "/images/favNoteGray.png"
                        }
                        width={30}
                        height={35}
                        alt="Favourite Icon"
