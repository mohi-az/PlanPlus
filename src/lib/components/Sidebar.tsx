  ...
  return (
    <div className="drawer lg:drawer-open h-full flex">
      ...
      <div className="fixed top-0">
        <label
          htmlFor="PlanPlusSlider"
          className="btn btn-ghost drawer-button text-xl pt-5 sm:pt-4 sm:text-3xl  lg:hidden text-white"
        >
          <RiMenuFold4Fill />
        </label>
      </div>
      <div className="drawer-side h-full ">
        ...
        <ul className="menu bg-base-200  text-base-content min-h-full w-80 p-4 text-sm md:text-base lg:text-lg">
          {menuItems.map((menu) => (
            <li
              key={menu.link}
              className={clsx(path === menu.link && "bg-base-300 font-bold")}
            >
              <Link href={menu.link}>
          ...
          {!isPending ? (
            categories
              .filter((ca) => ca.showInMenu === true)
              .map((ca) => (
                <li
                  key={ca.id}
                  className={clsx(
                    params.get("ca") === ca.id && "bg-base-300 font-bold",
                  )}
                >
                  <Link href={"/members/tasks/?ca=" + ca.id}>
                    <IconLoader name={ca.icon ? ca.icon : "FcWorkflow"} />
                    {ca.name}
                  </Link>
                </li>
              ))
